...
ul dsk_count(ul *floppies,ul *vdisks) {
   if (vdisks) {
      static int nvdisks = -1;
      // check "boot volume" presence
      if (nvdisks<0) nvdisks = hlp_disktype(QDSK_VOLUME|0)!=HDT_INVALID ? 1 : 0;
      *vdisks = nvdisks; //2; disk b: now is a ramfs
   }
...
ul dsk_write(ul disk, uq sector, ul count, void *data) {
   if (sector+count>0xFFFFFFFFLL) return 0;
   return dsk_action(disk, PDSK_WRITEPHYSTRACK, sector, count, data);
}
