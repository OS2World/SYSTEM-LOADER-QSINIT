   ...
   if (vd->vol>=0) {
      ...
      if (vdta->flags&VDTA_ON) {
         ...
         if ((vdta->flags&VDTA_FAT)!=0) {
            ...
            /* Not a FAT.
               Even if FSLIB is missing, HPFS format able to setup vdta in
               a way, compatible with "format" command printing and this
               function will return it, because nobody touched it until unmount */
            info->DataSector  = 0;
            info->FSizeLim    = 0;
         }
...
static void *m_list[] = { fat_initialize, fat_finalize, fat_avail, fat_volinfo,
   ...
   fat_close, fat_size, fat_setsize, fat_finfo, fat_setexattr, fat_getexattr,
   fat_lstexattr, fat_remove, fat_renmove, fat_makepath, fat_remove,
   fat_pathinfo, fat_setinfo, fat_diropen, fat_dirnext, fat_dirclose };

u32t register_fatio(void) {
   u32t   ii, cid;
   for (ii=0; ii<FF_VOLUMES; ii++) extdrv[ii] = (FATFS*)malloc(sizeof(FATFS));
   // register private(unnamed) class
   cid = exi_register("qs_sys_fatio", m_list, sizeof(m_list)/sizeof(void*),
                      sizeof(volume_data), EXIC_PRIVATE, fat_init, fat_done, 0);
   // register fs
   if (cid) {
      qserr res = io_registerfs(cid);
      if (res) {
         log_printf("fatfs: regfs err %08X\n", res);
         exi_unregister(cid);
         cid = 0;
      }
   }
