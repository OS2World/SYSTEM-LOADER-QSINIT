...
#ifndef __QSINIT__
   ushort bb[maxLineLength];
#else
   /* a fast way to catch buffer overflow
      memmgr will trap on free() */
   ushort*bb = (ushort*)malloc(maxLineLength*2);
#endif
   while (count-- > 0) {
      formatLine(bb, linePtr, delta.x+size.x, color);
      writeBuf(0, y, size.x, 1, bb+delta.x);
