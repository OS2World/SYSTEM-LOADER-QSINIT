...
static int dsk_clonechs(u32t dstdisk, disk_geo_data *srcgeo) {
...
qserr _std dsk_clonestruct(u32t dstdisk, u32t srcdisk, u32t flags) {
   ...
   u64t        reqlen, 
            firstused;
   // same disk?
   if (dstdisk==srcdisk) return E_SYS_INVPARM;
   FUNC_LOCK       lk;
   hS = get_by_disk(srcdisk);
   hD = get_by_disk(dstdisk);
   if (!hS || !hD) return E_DSK_DISKNUM;
   // force rescan of both disks
   rc = dsk_ptrescan(dstdisk, 1);
   if (rc && rc!=E_PTE_EMPTY) return rc;
   if (!rc) return E_PTE_CSPACE;
   rc = dsk_ptrescan(srcdisk, 1);
   if (rc) return rc;
   // get sorted disk map 
   map = dsk_getmap(srcdisk);
   if (!map) return E_DSK_DISKNUM;
   // hybrid, too crazy to clone ;)
   if (hS->hybrid) return E_PTE_HYBRID;
   // sector size mismatch, fatal.
   if (hS->info.SectorSize!=hD->info.SectorSize) return E_PTE_INCOMPAT;
   ...
   do {
      // no space
      if (reqlen>hD->info.TotalSectors) { rc = E_PTE_CSPACE; break; }
      ...
      if ((flags&DCLN_IDENT)) {
         ...
         if ((flags&DCLN_SAMEIDS)==0) {
            ...
            if (dsk_sectortype(dstdisk,0,(u8t*)mbr)==DSKST_PTABLE) {
               ...
               if (!hlp_diskwrite(dstdisk, 0, 1, mbr)) rc = E_DSK_ERRWRITE;
            }
            free(mbr);
         }
      } else {
      ...
      if ((flags&DCLN_IDENT)==0 && (flags&(DCLN_SAMEIDS|DCLN_MBRCODE))) {
         ...
         if (!rc) {
            ...
            if (!hlp_diskwrite(dstdisk, 0, 1, dmbr)) rc = E_DSK_ERRWRITE;
         }
         free(dmbr); free(smbr);
      }
      if (rc) break;
...
qserr _std dsk_clonedata(u32t dstdisk, u32t dstindex, u32t srcdisk, 
   u32t srcindex, read_callback cbprint, u32t flags)
{
   ...
   if (dstdisk==srcdisk && dstindex==srcindex) return E_SYS_INVPARM; else {
      FUNC_LOCK    lk;
      hdd_info    *hS, *hD;
      ...
      sector_sz = hS->info.SectorSize;
      // sector size mismatch, fatal.
      if (hS->info.SectorSize!=hD->info.SectorSize) return E_PTE_INCOMPAT;
      // query size/pos
      stype = dsk_ptquery64(srcdisk, srcindex, &spos, &slen, 0, 0);
      if (stype==0) return E_PTE_PINDEX;
      dtype = dsk_ptquery64(dstdisk, dstindex, &dpos, &dlen, 0, &dflags);
      if (dtype==0) return E_PTE_PINDEX;
      if (slen>dlen) return E_PTE_CSPACE;
      // may be sometimes later ;)
      if (slen>=_4GBLL) return E_PTE_LARGE;
      
      /* check destination - is it mounted?
         deny boot partition and unmount any other */
      svol = dsk_ismounted(dstdisk, dpos);
      if (svol==QDSK_VOLUME) return E_PTE_BOOTPT;
      if (svol&QDSK_VOLUME) 
         if (!hlp_unmountvol(svol&QDSK_DISKMASK)) return E_DSK_UMOUNTERR;
   }
   ...
   if ((flags&DCLD_SKIPBPB)==0) {
      ...
      if (bstype==DSKST_BOOTFAT || bstype==DSKST_BOOTBPB) {
         ...
         // update BPB
         if (wrt)
            if (!hlp_diskwrite(dstdisk, dpos, 1, br)) rc = E_DSK_ERRWRITE;
      } else
      if (bstype==DSKST_BOOTEXF) {
         ...
         if (bre->BR_ExF_VolStart!=dpos) {
            ...
            if (!exf_updatevbr(dstdisk, dpos, bre, 1, 0)) rc = E_DSK_ERRWRITE;
         }
      }
   }
